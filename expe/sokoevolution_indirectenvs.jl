...
# function log_gen(e::sNES{ContinuousSokoLvl})
...
function get_local_evaluation(envs::sNES{ContinuousSokoLvl},agents::sNES{SokoAgent})
    ...
    for i in 1:envs_size
        ...
        for j in 1:agents_size
            local_eval[i,j] = play_lvl(lvl_str,agents.population[j])
        end
    end
    return local_eval
end
...
#------------------------------------Main--------------------------------------#
envs = sNES{ContinuousSokoLvl}(envs_model,cfg_envs,fitness_env;logfile=string("logs/","$expe_name/envs", ".csv"))
