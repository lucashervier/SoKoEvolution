...
function plot_results(title_plot,gen_start,gen_step,gen_end,figure_saving_path)
    ...
    plot(xs,background=:lightgray,xaxis="Generation number",yaxis="Fitness",title=title_plot);
    plot!(xs,fitness_mean_envs[Int(gen_start/gen_step):Int(gen_end/gen_step)],ribbon=fitness_std_envs[Int(gen_start/gen_step):Int(gen_end/gen_step)],fillalpha=.5,label="Envs fitness");
