...
expe_name = "envs_hyperparameter4"
...
function fitness_env(env::ContinuousSokoLvl)
    ...
    nb_boxes = count_items(1,first_observation)
    nb_holes = count_items(4,first_observation)
    ...
    return [agent_reward,random_reward,nb_boxes,nb_holes,initial_connectivity_number,no_boxes_moved]
end
...
function evaluate_with_detailed(e::AbstractEvolution)
    ...
    for i in eachindex(e.population)
        ...
        fitness = 0
        nb_objectives = min(detailed_fitness[3],detailed_fitness[4])
        if nb_objectives == 0
            fitness = -10
        elseif detailed_fitness[5] == 0
            fitness = -10
        else
            fitness = (detailed_fitness[1]-detailed_fitness[2])/nb_objectives -
                        (detailed_fitness[5]>7) -2*detailed_fitness[6]
                        -(detailed_fitness[3]-detailed_fitness[4])^2
        end
