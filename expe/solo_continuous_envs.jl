...
function save_gen(e::sNES{ContinuousSokoLvl})
    path = Formatting.format("gens/solo_continuous_env/{1:04d}", e.gen)
    mkpath(path)
    sort!(e.population)
    for i in eachindex(e.population)
...
#------------------------------------Main------------------------------------#
envs = sNES{ContinuousSokoLvl}(envs_model,cfg_envs,fitness_env;logfile=string("logs/","solo_continuous_envs/solo_continuous_envs", ".csv"))
